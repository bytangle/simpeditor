...
impl TextViewer {
  ...
  pub fn show_document(&mut self) {
    ...
    if self.doc_length < self.terminal_size.height {
      for line in 0..self.doc_length {
        println!("{}\r", self.doc.lines[line as usize]);
      }   
    } else {
      if pos.y <= self.terminal_size.height {
        for line in 0..self.terminal_size.height - 3 {
          println!("{}\r", self.doc.lines[line as usize]);
        }
      } else {
        for line in pos.y - (self.terminal_size.height - 3)..pos.y {
          println!("{}\r", self.doc.lines[line as usize])
        }
      }
    }
  ...
  fn inc_x(&mut self) {
    if self.current_position.x < self.terminal_size.width {
      self.current_position.x += 1;
    }
  ...
  fn inc_y(&mut self) {
    if self.terminal_size.height > self.current_position.y {
      self.current_position.y += 1;
    }
